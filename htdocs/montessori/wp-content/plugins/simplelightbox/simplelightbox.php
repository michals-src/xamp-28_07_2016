...
class SimpleLightbox {
	...
	private function __construct() {
		...
		foreach($options as $k => $v) {
			$checkboxes = array(
				...
				'ar_sl_swipeClose',
				'ar_sl_counter',
				'ar_sl_animationSlide',
				'ar_sl_preloading',
				'ar_sl_enableKeyboard',
				'ar_sl_loop',
				'ar_sl_docClose',
			...
			if(in_array($k, $checkboxes) && get_option($k) === ''){
				$this->options[$k] = 0;
			}
		}
	...
	public function page_init() {
		...
		$fields = array(
			...
			'ar_sl_captionDelay' => array(
				...
				'desc' => __('Time in ms, the show up for the caption should be delayed', 'simplelightbox'),
			),
			'ar_sl_close' => array(
				'type'  => 'checkbox',
			...
			'ar_sl_swipeClose' => array(
				'type'  => 'checkbox',
				'label' => __('swipeClose', 'simplelightbox'),
			...
			'ar_sl_animationSlide' => array(
				'type'  => 'checkbox',
				'label' => __('Animation-Slide', 'simplelightbox'),
			...
			'ar_sl_alertError' => array(
				...
				'desc' => __('show an alert, if image was not found','simplelightbox')),
			'ar_sl_alertErrorMessage' => array(
				'type'  => 'text',
				'label' => __('Alert Error Message', 'simplelightbox'),
				'desc'  => __('the message displayed if image was not found', 'simplelightbox')
			),
			'ar_sl_history' => array(
				'type'  => 'checkbox',
				'label' => __('Enable History', 'simplelightbox'),
				'desc'  => __('enable history back closes lightbox not page reload', 'simplelightbox')
			),
	...
	public function load() {
		wp_enqueue_script('simplelightbox', plugins_url('/dist/simple-lightbox.min.js', __FILE__), array('jquery'), '1.10.0', true);

		//== simplelightbox JS hook
		wp_register_script('simplelightbox-call', plugins_url('/resources/js/setup.simplelightbox.js', __FILE__), array('jquery', 'simplelightbox'), '1.10.0', true);
	...
	public function load_admin_styles() {
		...
		//== js
		wp_enqueue_script('simplelightbox-colorpicker', plugins_url('/resources/js/colorpicker.min.js', __FILE__), array( 'jquery' ), '1.10.0', true);
		wp_enqueue_script('simplelightbox-admin-js', plugins_url('/resources/js/ar-sl-admin.js', __FILE__), array( 'jquery' ), '1.10.0', true);
	}
