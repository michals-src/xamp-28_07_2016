	...
	function customize_register( $wp_customize ){
		...
		if( method_exists($wp_customize, 'add_panel') ) {
			$wp_customize->add_panel( 'page_settings', array(
				'title' => __( 'Page Template Settings', 'siteorigin-unwind' ),
				'description' => __( 'Change layouts for various pages on your site.', 'siteorigin-unwind' ),
				'priority' => 11,
			) );
		}
		...
		$types = array();
		$templates = array(
			'home' => __( 'Blog Page', 'siteorigin-unwind' ),
			'search' => __( 'Search Results', 'siteorigin-unwind' ),
			'date' => __( 'Date Archives', 'siteorigin-unwind' ),
			'author' => __( 'Author Archives', 'siteorigin-unwind' ),
			'404' => __( 'Not Found', 'siteorigin-unwind' ),
		);
		...
		foreach( $post_types as $post_type => $post_type_data ) {
			...
			$types[] = array(
				'group' => 'archive',
				'id' => $post_type,
				'title' => __( 'Type', 'siteorigin-unwind' ) . ': ' . $post_type_data->label
			);
		}
		...
		foreach( $taxonomies as $tax_slug => $taxonomy ) {
			...
			$types[] = array(
				'group' => 'taxonomy',
				'id' => $tax_slug,
				'title' => __( 'Taxonomy', 'siteorigin-unwind' ) . ': ' . $taxonomy->label
			);
		}
