	...
	*/
	...
	enedScroll.prototype.helpers = function(){
		...
		plugins.pin = function( target ){
			...
			controller.setParent = function( parentHeight, timeline ){
				...
				// var className = 'elementParent1234';

				// if( $(document).find('.' + className).length === 0 ){
				...
				// Parent.style.position = 'relative';
				// Parent.style.paddingTop = '0px';
				// Parent.style.paddingBottom = parentHeight + 'px';
				...
				// controller.parentHeight = parentHeight;
			...
			controller.route = function( elements ){
				...
				for( var keys in elements ){
					...
					//var abs = ( ( ! item.hasBefore && key == 0 ) ) ? 0 : (item.offset - );
					var abc = ( ( ! item.hasBefore && key == 0 ) ) ? 0 : 50;

					//console.log( $(controller.target).offset().top );
					//console.log( $(controller.target).offset().top - TRIGGER.progress() );
	...
	var Interface = enedScroll.Interface = (function(window){
		...
		*
		 ...
		 * all of needed elements to an array
		 *
		 * @return boolean
		 */
