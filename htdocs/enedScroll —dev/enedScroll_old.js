...
(function( root, factory ) {
	...
	*/
	...
	enedScroll.prototype.helpers = function(){
		 ...
		 * @return function
		 */
		plugins.value = function(options){
			var output = {};
			...
			output.before = function(){ 
				if(before){
					addStyle(self.client.parent(), {
						'padding-bottom': '0'
					});
				}
			};
			output.in = function(){	
				//console.log(onUpdateHelper);
				
					// addStyle(self.client.parent(), {
					// 	'padding-bottom': (355 * onUpdateHelper) + 'px'
					// });
				console.log('aaa');
		...
		plugins.value2 = function(options){
			var output = {};
				myName = self.abstract(options),
				onUpdateHelper = self.onUpdateHelper(myName);
			...
			output.in = function(){
				...
				addStyle(self.client.parent(), {
					'padding-bottom': (355 - (355 * onUpdateHelper)) + 'px'
				});
				
				//console.log(Interface.get.height(self.client.parent()));
			};
			output.after = function(){ 
				if(after){
					addStyle(self.client.parent(), {
