 ...
 * By Cameolon
 *
 * @documentation {{ url_main_docs }}
 * @author Cameolon {{ email@address }}
 * @version 1.5.0
 *
 * Released under Envato license.
 ...
 */
 	...
 	var __ = function( item ){
 	   ...
 	   */
	 	utils.offsetTop = function(){
	 		return ( el && typeof el.scrollTop === 'number' && el != window ) ? el.scrollTop : window.pageYOffset;
	 	}
 			...
 			if( utils.isType( "string" ) ){
 				...
 				if( verification !== null && verification && verification.length === 1 ){
 					...
 					if( percent > 0 && percent <= 100 ){
 						...
 						//throw wrong number
 						LOG( "enedScroll_error: Converting value is incorrect, should be between 1 - 100.", 3 );
 					}
	 ...
	 * 
	 */
	var LOAD_EVENT_ENVIRONMENT = document;
	var LOAD_EVENT = function(callback){
		...
		LOAD_EVENT_ENVIRONMENT.addEventListener( 'scroll', callback );
		LOAD_EVENT_ENVIRONMENT.addEventListener( 'resize', callback );
		LOAD_EVENT_ENVIRONMENT.addEventListener( 'mousewheel', callback );
		LOAD_EVENT_ENVIRONMENT.addEventListener( 'DOMmouseScroll', callback );
	...
	var VISUAL = {
		...
		TRIGGER: function( options, properties ){
			...
			TRIGGER_VISUAL.style.left = ( $(properties.environment.element).width() - $(TRIGGER_VISUAL).width() ) + "px";

			if( properties.environment.element != window ){

				TRIGGER_VISUAL.style.top = parseInt( TRIGGER_VISUAL.style.top, 10 ) - window.pageYOffset + "px";
				TRIGGER_VISUAL.style.left = parseInt( TRIGGER_VISUAL.style.left, 10 ) + $(properties.environment.element).offset().left + "px";

			}

			return TRIGGER_VISUAL;
		 ...
		 * 
		 */
		DRAW: function( options, properties ){
			...
			var PARENT = function(){
				...
				//VISUALISATION_PARENT.style.width = ( $(properties.environment.element).width() - properties.environment.offsetLeft ) + "px";
				VISUALISATION_PARENT.style.width = "100%";
			...
			var SELECTORS = {
				TOP_SELECTOR: function(){
					...
					PARENT.style.display = "block";
		 			...
		 			//PARENT.style.width = $(properties.environment.element).width() + 'px';
		 			...
		 			//SELECTOR.style.width = ( $(properties.environment.element).width() - 15 ) + 'px';
			...
			if( options.show_selectors === true ){
		 		if( VISUALISATION_PARENT === null ){
			 		...
			 		if( $(document).find( $(VISUALISATION_PARENT) ).length <= 0 && __(properties).isType("object") ){
			 			var target = ( properties.environment.element != window ) ? properties.environment.element : "body";
				 		$(VISUALISATION_PARENT).appendTo( $(target) );
				 	}
		 			
				 	return { "PARENT": VISUALISATION_PARENT, "TOP_SELECTOR": TOP_SELECTOR, "BOTTOM_SELECTOR": BOTTOM_SELECTOR };

		 		}
		 	}

		 	return { "PARENT": null, "TOP_SELECTOR": null, "BOTTOM_SELECTOR": null };
		...
		/**
		 *
		 * determines the position of the elements
		 * if the enviroment is not a window
		 * 
		 * @object options
		 * 
		 */
		LOCATION: function( elements, properties ){

			var VISUALISATION_ELEMENTS = ( elements && __(elements).isType("object") ) ? elements : null;
			
			if( VISUALISATION_ELEMENTS == null){
				return;
			}
			...
			if( properties.environment.element != window ){
				...
				LOAD_EVENT(function(){
					...
					//VISUALISATION_ELEMENTS.SELECTORS.PARENT.style.top -= properties.progress;
					//VISUALISATION_ELEMENTS.TRIGGER.style.top -= window.pageYOffset + "px";

				});

				var trigger_location = function(){
					VISUALISATION_ELEMENTS.TRIGGER.style.top = parseInt(properties.marginTop, 10) - window.pageYOffset + "px";
				};

				window.addEventListener( 'scroll', trigger_location );
				window.addEventListener( 'resize', trigger_location );
				window.addEventListener( 'mousewheel', trigger_location );
				window.addEventListener( 'DOMmouseScroll', trigger_location );
	 ...
	 *
 	...
 	var enedScroll = function( scene, trigger, options ){
 		...
 		$(document).ready(function(){
 			...
 			VISUALISATION.client = {
 				offsetTop: ( $(CLIENT).offset().top - TRIGGER.environment.offsetTop ),
 				height: $(CLIENT).outerHeight(),
 			};

 			VISUALISATION.name_selectors = "Scene " + VISUALISATION.name_selectors;

 			//var selectors_visual = VISUAL.DRAW( VISUALISATION, TRIGGER );
 			var trigger_visual = VISUAL.TRIGGER( VISUALISATION, TRIGGER );

 			//var elements_visual = { "TRIGGER": trigger_visual, "SELECTORS": selectors_visual };
 			var elements_visual = { "TRIGGER": trigger_visual };

 			VISUAL.LOCATION( elements_visual, TRIGGER );
 	...
 	/**
 	 *
	 * Determine an element from the properties are taken.
	 * First found item is used so use unique name of element.
	 *
	 * var environment = new enedScroll.Environment( @name )
	 * 
	 * @(string|object) name => default: window
	 *
	 * url_docs
	 * v1.0.0
	 *
 	 */
 	enedScroll.Environment = function( name ){

 		"use strict";

 		var PROPERTIES = {};

 		PROPERTIES.element 		= ( name && __(name).isType(["object", "string"]) && name != window ) ? $( name )[0] : window;
 		PROPERTIES.offsetTop 	= ( PROPERTIES.element != window ) ? $(PROPERTIES.element).offset().top : 0;
 		PROPERTIES.offsetLeft 	= ( PROPERTIES.element != window ) ? $(PROPERTIES.element).offset().left : 0;

 		if( PROPERTIES.element != window ){
 			$(PROPERTIES.element).css( "position", "relative" );
 			LOAD_EVENT_ENVIRONMENT = PROPERTIES.element;
 		}else{
 			LOAD_EVENT_ENVIRONMENT = document;
 		}
 			
 		return PROPERTIES;
 		
 	}; // enedScroll.Environment
	 ...
	 * @enedScroll.environment environment => default: window
	 *
	 * url_docs
	 * v1.0.1
	 *
 	 */
 	enedScroll.Trigger = function( name, options, environment ){
 		...
 		PROPERTIES.environment 	= ( environment && __(environment).isType( [ "object" ] ) ) ? environment : new enedScroll.Environment(window);
 		PROPERTIES.marginTop 	= ( options && __(options).isType( [ "number", "string" ] ) ) ? parseInt( ( options + PROPERTIES.environment.offsetTop), 10 ) : PROPERTIES.environment.offsetTop;
 		PROPERTIES.progress 	= __( PROPERTIES.environment.element ).offsetTop() + PROPERTIES.marginTop;
	 ...
	 *
 	...
 	enedScroll.prototype.__abstract = function( duration, o ){
 		...
 		VISUALISATION.client = {
 			offsetTop: ( offsetTop() - $self.trigger.environment.offsetTop ),
 			height: ( __( duration ).getDuration( $self.client ) ),
 		};


 		VISUAL.DRAW( VISUALISATION, $self.trigger );
	 ...
	 *
 	...
 	enedScroll.prototype.pin = function( element, timeline ){
 		...
 		var correctingLocation = function(){ 
			for( var pin_key = 0 ; pin_key < pintable.length; pin_key++ ){
				...
				if( pintable[pin_key]["PIN_ABSTRACT"].state === "BEFORE" ||
					...
					window.removeEventListener( 'scroll', correctingLocation );
					window.removeEventListener( 'resize', correctingLocation );
					window.removeEventListener( 'mousewheel', correctingLocation );
					window.removeEventListener( 'DOMmouseScroll', correctingLocation );
				}
				...
				if( pintable[pin_key]["PIN_ABSTRACT"].state === "DURING" ){
					...
					target[0].style.top = (target_pos_top + ( $self.trigger.progress - ABSTRACT_SELECTOR_TOP )) - window.pageYOffset + 'px';
				}			
 			...
 			LOAD_EVENT(function(){
 				...
 				for( var pin_key = 0 ; pin_key < pintable.length; pin_key++ ){
 					...
 					if( pintable[pin_key]["PIN_ABSTRACT"].state === "DURING" ){
 						...
 						if( $self.trigger.environment.element == window ){
 							/* WINDOW */
							target[0].style.top = (( target_pos_top - __( $self.trigger.environment.element ).offsetTop() ) + ( $self.trigger.progress - ABSTRACT_SELECTOR_TOP ) ) + 'px';
 						}else{
							...
							window.addEventListener( 'scroll', correctingLocation );
							window.addEventListener( 'resize', correctingLocation );
							window.addEventListener( 'mousewheel', correctingLocation );
							window.addEventListener( 'DOMmouseScroll', correctingLocation );

 						}
 						
 						
 						//console.log( ( target_pos_top + ( $self.trigger.progress - ABSTRACT_SELECTOR_TOP ) ) - $(window).pageYOffset );
 						//console.log( ( target_pos_top - __( $self.trigger.environment.element ).offsetTop() ) + ( $self.trigger.progress - ABSTRACT_SELECTOR_TOP ) );
