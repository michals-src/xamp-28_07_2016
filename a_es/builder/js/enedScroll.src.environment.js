 ...
 */
 	...
 	var __ = function( item ){
 	   ...
 	   */
	 	utils.offsetTop = function(){
	 		return ( el && el != window ) ? el.scrollTop() : window.pageYOffset;
	 	}
	...
	var VISUAL = {
		...
		TRIGGER: function( options, properties ){
			...
			TRIGGER_VISUAL.style.top = ( properties.offsetTop + properties.marginTop ) + 'px';
			TRIGGER_VISUAL.style.right = "0px";
			
			if( options.environment != "body" ){
				TRIGGER_VISUAL.style.right = $(window).width() - ( properties.offsetLeft + $(options.environment).width() ) + "px";
			}
			...
			if( $(document).find( $(TRIGGER_VISUAL) ).length <= 0 ){
				$(TRIGGER_VISUAL).appendTo( $( options.environment ) );
			}
		...
		DRAW: function( options ){
			...
			var PARENT = function(){
				...
				VISUALISATION_PARENT.style.width = $( options.environment ).width() + "px";
			...
			var SELECTORS = {
				TOP_SELECTOR: function(){
					...
					PARENT.style.display = "block";
		 			...
		 			PARENT.style.width = $( options.environment ).width() + 'px';
		 			PARENT.style.top = ( 0 + options.selectors[0] ) + 'px';
		 			PARENT.style.zIndex = options.zIndex_selectors;

		 			// TOP
		 			SELECTOR.style.display = "block";
		 			SELECTOR.style.width = ( $( options.environment ).width() - 15 ) + 'px';
			...
			if( options.show_selectors === true ){
		 		if( VISUALISATION_PARENT === null ){
			 		...
			 		if( $(document).find( $(VISUALISATION_PARENT) ).length <= 0 ){
				 		$(VISUALISATION_PARENT).appendTo( $( options.environment ) );
				 	}
	...
	var LOAD_EVENT = function(callback, environment){
 	...
 	var enedScroll = function( scene, trigger, options ){
 		...
 		$(document).ready(function(){
 			...
 			VISUALISATION.client = {
 				offsetTop: ( $(CLIENT).offset().top - TRIGGER.offsetTop ),
 				height: $(CLIENT).outerHeight(),
 			};

 			VISUALISATION.name_selectors = "Scene " + VISUALISATION.name_selectors;
 			VISUALISATION.environment = ( TRIGGER.environment != window ) ? TRIGGER.environment : 'body';
	 ...
	 *
 	...
 	enedScroll.Trigger = function( name, options, environment ){
 		...
 		PROPERTIES.environment 	= ( __(environment).isType(["string", "object"]) && environment != window ) ? $(environment) : window;
 		PROPERTIES.offsetTop 	= ( __(environment).isType(["string", "object"]) && environment != window ) ? $(PROPERTIES.environment).offset().top : 0;
 		PROPERTIES.offsetLeft 	= ( __(environment).isType(["string", "object"]) && environment != window ) ? $(PROPERTIES.environment).offset().left : 0;

 		if( PROPERTIES.environment != window ){
 			$(PROPERTIES.environment).css( "position", "relative" );
 			LOAD_EVENT_ENVIRONMENT = $(PROPERTIES.environment)[0];
 		}

 		PROPERTIES.progress 	= __( PROPERTIES.environment ).offsetTop() + PROPERTIES.marginTop;
	 ...
	 *
 	...
 	enedScroll.prototype.__client = function( client, selectors ){
 		...
 		var offsetTop = function(){
 			return ( $(client).offset().top - $self.trigger.offsetTop ) + selectors[0];
 		}
 		...
 		var offsetBottom = function(){
 			return ( $(client).offset().top - $self.trigger.offsetTop ) + height() - selectors[1];
 		} 
	 ...
	 *
 	...
 	enedScroll.prototype.pin = function( element, timeline ){
 		...
 		var createParent = function( parent_height, parent_padding ){
			...
			Parent.style.minWidth = $(element).width() + 'px';
 			...
 			LOAD_EVENT(function(){
 				...
 				for( var pin_key = 0 ; pin_key < pintable.length; pin_key++ ){
 					...
 					if( pintable[pin_key]["PIN_ABSTRACT"].state === "DURING" ){
 						...
 						target[0].style.top = ( ( target_pos_top - $( $self.trigger.environment ).scrollTop() ) + ( ( __( $self.trigger.environment ).offsetTop() + $self.trigger.marginTop ) - ABSTRACT_SELECTOR_TOP ) ) + 'px';
 						
 						if( $self.trigger.environment != window ){
							
							target[0].style.top = (( ( target_pos_top + $( $self.trigger.environment ).scrollTop() ) - __( $self.trigger.environment ).offsetTop() ) + ( ( __( $self.trigger.environment ).offsetTop() + $self.trigger.marginTop ) - ABSTRACT_SELECTOR_TOP ) ) + 'px';
